...
export default async function BudgetsPage({
  ...
  function renderRow(category: (typeof expenseCategories)[number]) {
    ...
    // getBudgets carries a budget row forward from whichever month it was
    // last saved in (including its stored amount_limit) — fine for a fixed
    // manual amount, but an "auto" budget must stay live: recompute from
    // this month's actual bills every time rather than replaying a stale
    // saved figure, and treat "no row at all but bills exist" the same way.
    const liveBillTotal = categoryBills.reduce((s, b) => s + b.effectiveAmount, 0);
    const isAuto = budget ? budget.auto : liveBillTotal > 0;
    ...
    const limit = rolling ? rolling.cumulativeTotal : isAuto ? liveBillTotal : (budget?.amount_limit ?? null);
