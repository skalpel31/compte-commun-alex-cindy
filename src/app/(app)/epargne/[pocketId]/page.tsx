...
import { WithdrawSavingsSheet } from "@/components/withdraw-savings-sheet";
...
export default async function SavingsPocketPage({
  ...
  const total = contributions.reduce(
    (sum, t) => sum + (t.category?.type === "income" ? t.amount : -t.amount),
    0
  );
      ...
      <Card className="glass">
        <CardContent className="flex flex-col gap-3 pt-4">
          <div className="flex items-center justify-between gap-3">
            ...
            <WithdrawSavingsSheet pocketId={pocket.id} />
          </div>
        ...
        <CardContent className="divide-y">
          {contributions.length === 0 ? (
            <p className="py-8 text-center text-sm text-muted-foreground">
              Aucun mouvement enregistré sur ce compte pour l&apos;instant.
            </p>
          ) : (
            contributions.map((t) => {
              const isDeposit = t.category?.type === "income";
              return (
                <div key={t.id} className="flex items-center justify-between gap-3 py-3">
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium">
                      {t.description || (isDeposit ? "Versement" : "Retrait")}
                    </p>
                  ...
                  <p
                    className={`shrink-0 text-sm font-semibold tabular-nums ${
                      isDeposit ? "text-good" : "text-critical"
                    }`}
                  >
                    {isDeposit ? "+" : "-"}
